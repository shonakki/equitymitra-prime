...
function USAIndexCard({ symbol, label, ltp, pct }: { symbol: string; label: string; ltp: number; pct: number }) {
  return (
    <div className="rounded-xl border border-[var(--gold)]/25 bg-gradient-to-b from-[var(--gold)]/10 to-card/60 p-5 flex flex-col justify-between">
      <div>
        <div className="flex items-center justify-between mb-2">
          <p className="text-[10px] uppercase tracking-wider font-semibold text-white/70">{symbol}</p>
        </div>
        <p className="text-2xl font-bold text-white">{ltp.toLocaleString('en-US', { maximumFractionDigits: 2 })}</p>
        <div className="flex items-center gap-1 mt-2">
          <span className={`text-sm font-semibold ${pct >= 0 ? "text-emerald-400" : "text-red-400"}`}>
            {pct >= 0 ? "+" : ""}{pct.toFixed(2)}%
          </span>
        </div>
      </div>
      <p className="text-[10px] text-white/40 mt-3">{label}</p>
    </div>
  );
}
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 py-6 space-y-6">
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {loading ? (
              ...
              <div className="col-span-1 md:col-span-3 text-xs text-red-400 bg-red-500/10 border border-red-500/25 rounded p-4">
                Failed to load USA indices: {error}
              </div>
            ) : data && "indices" in data && data.indices ? (
              <>
                {data.indices.slice(0, 2).map((idx) => (
                  <USAIndexCard
                    key={idx.key}
                    symbol={idx.symbol}
                    label={idx.key}
                    ltp={idx.ltp || 0}
                    pct={idx.percentChange || 0}
                  />
                ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            ) : error ? null : data && "indices" in data && data.indices && data.indices.length > 2 ? (
              <>
                {data.indices.slice(2).map((idx) => (
                  <USAIndexCard
                    key={idx.key}
                    symbol={idx.symbol}
                    label={idx.key}
          ...
          {/* Row 3: Gainers + Losers + Earnings Calendar */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {/* Top Gainers */}
            <div className="rounded-xl border border-white/10 bg-card/60 p-5">
              <div className="flex items-center justify-between mb-3">
            ...
            <div className="rounded-xl border border-white/10 bg-card/60 p-5 flex flex-col">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-semibold text-white flex items-center gap-1.5">
                  <Calendar className="h-4 w-4 text-[var(--gold)]" /> Earnings Calendar
                </h3>
                {loading && <Skeleton className="h-3.5 w-12" />}
              </div>
              ...
              ) : data && (!("earnings" in data) || data.earnings === null || (data.earnings && data.earnings.length === 0)) ? (
                <div className="flex-1 flex flex-col items-center justify-center py-6 text-center text-xs text-white/40">
                  <Calendar className="h-8 w-8 stroke-[1.5] mb-2 text-white/20" />
                  <p className="font-medium text-white/60">Earnings Data Unavailable</p>
                </div>
              ) : data && "earnings" in data && data.earnings ? (
                <ul className="space-y-3 overflow-y-auto max-h-[220px] flex-1 pr-1 scrollbar-thin">
                  {data.earnings.slice(0, 10).map((earning) => (
                    <li key={earning.name} className="rounded-md bg-white/5 p-3 flex flex-col justify-between">
                      <div>
                        <div className="flex items-start justify-between gap-2">
                          <p className="text-[11px] font-semibold text-white leading-tight">{earning.name}</p>
                          <span className={`text-[9px] px-1.5 py-0.5 rounded font-medium ${
                            earning.status === "LIVE" ? "bg-emerald-500/15 text-emerald-400 border border-emerald-500/25" :
                            earning.status === "COMPLETED" ? "bg-white/10 text-white/60" :
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="rounded-xl border border-white/10 bg-card/60 p-5 flex flex-col">
          ...
          ) : data && (!data.ipos || data.ipos.length === 0) ? (
            <div className="flex-1 flex flex-col items-center justify-center py-6 text-center text-xs text-white/40">
              <Calendar className="h-8 w-8 stroke-[1.5] mb-2 text-white/20" />
              <p className="font-medium text-white/60">IPO Data Unavailable</p>
            </div>
